...
> **🗓️ تحديث التحويل للإنجليزي الخالص (لرفعها على Upwork/Freelancer):** طبقة i18n أُزيلت بالكامل (`translations.js` حُذفت، لا زر تبديل، لا LocalStorage). الموقع الآن **English-only** ثابت: `lang="en" dir="ltr"`، كل النصوص/CSS مضمّنة بالإنجليزي، خط `Cairo` أُزيل، والـ bundle أصغر (20 module / 9.32 kB gzip). أقسام التقرير المتعلقة بالتبديل/RTL أدناه تاريخية عمدًا.
...
| 5 | ~~التبديل AR/EN~~ — الطبيعة الحالية: ثابتة إنجليزي (`lang=en dir=ltr`) بلا تبديل | ✅ |
...
| `button.lang-toggle` | ~~التبديل AR↔EN~~ — **أُزيل** في تحويل English-only | ✅ غير موجود |
...
| `html lang=ar dir=rtl` (ويتحول لـ `en/ltr`) | ✅ → **الآن ثابتة `lang=en dir=ltr`** بعد التحويل |
...
|---|---|---|
| تحميل الصفحة + العنوان + `h1` | ✅ | بالإنجليزي (بعد تحويل English-only) |
| العدّادات إلى الأرقام النهائية + `reduced-motion` | ✅ | 2,400 / 150 / 9 / 48 والمجموع 7,300 |
| طباعة الختم «مُقفَل» | ✅ | |
| كل روابط النف + كل CTAs | ✅ | تصل للأقسام الصحيحة |
| Full — تبديل اللغة | ✅ ~~تبديل~~ → **English-only** (`lang=en dir=ltr`)، بدون زر | التحويل الجديد |
